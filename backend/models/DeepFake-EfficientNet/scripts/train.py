...
def main():
    ...
    # Setup logger
    logger = setup_logger(
        name="training", log_file=str(log_dir / "training.log"), level="INFO"
    )
    ...
    # --- Training Dataset ---
    train_ds_real = DeepFakeDataset(
        train_real_config, is_real=True, transform=train_transforms
    )
    train_ds_fake = DeepFakeDataset(
        train_fake_config, is_real=False, transform=train_transforms
    )
    train_dataset = ConcatDataset([train_ds_real, train_ds_fake])

    # --- Validation Dataset ---
    val_ds_real = DeepFakeDataset(
        val_real_config, is_real=True, transform=val_transforms
    )
    val_ds_fake = DeepFakeDataset(
        val_fake_config, is_real=False, transform=val_transforms
    )
    val_dataset = ConcatDataset([val_ds_real, val_ds_fake])
